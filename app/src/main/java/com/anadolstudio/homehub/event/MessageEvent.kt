...
internal data class InterstitialMessageEvent(val message: Text, val snackbarStyle: SnackbarStyle) : Event
...
internal fun EventsDispatcher.showMessage(
    ...
    showEvent(MessageEvent(Text.Plain(message), snackbarStyle))
}
...
internal fun EventsDispatcher.showMessage(
    ...
    showEvent(MessageEvent(text, snackbarStyle))
}
...
internal fun EventsDispatcher.showMessage(
    ...
    showEvent(MessageEvent(Text.Resource(resourceId), snackbarStyle))
}
...
internal fun EventsDispatcher.showTodo() {
    val message = todoMessages.random()
    if (BuildConfig.DEBUG) Timber.d("showTodo: $message")

    showEvent(MessageEvent(Text.Plain(message), SnackbarStyle.Default))
}
