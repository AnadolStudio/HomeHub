    ...
    private fun loadHistory(device: HomeAssistantDevice, loadingContext: LoadingContext) {
        ...
        historyJob = lceFlow {
            ...
            buildHistoryEntries(device, historyByEntity)
                ...
                .onEachProgressState(
                        previousState = state.historyState.progressState,
                        loadingContext = loadingContext,
                        onNewProgressState = { newProgress ->
                            updateState { copy(historyState = historyState.copy(progressState = newProgress)) }
                        },
                )
                .onEachContent { (entries, total) ->
                    updateState {
                        copy(historyState = historyState.copy(entries = entries, totalCount = total))
                    }
                }
                .onEachError {
                    showError(it)
        ...
        val flatEntries = historyByEntity
                ...
                .mapNotNull { historyState ->
                    ...
                    DeviceHistoryEntry(
                            ...
                            value = displayValue,
                            timestamp = historyState.lastChanged,
